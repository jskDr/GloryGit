...
인버스 (INVERSE): 명령어를 뒤에서부터 해석한다 (한글 방식).
리버스 (REVERSE): 명령어를 원래처럼 앞에서부터 해석한다 (영어 방식).
...
class BASIC(cmd.Cmd):
	...
	def do_MODE( self, line):
		...
		if self.mode == 'kor_only':
			print( '한글 전용 모드이다.')
			print( '도움말을 입력하면 세부 정보를 보여준다.')
	...
	def line_rsplit(self, line):
		...
		if line_cmd == '찍어라':
			# 어미를 처리한다. 사실 제거하는 형태이다.
			if line_tail.endswith('를') or line_tail.endswith('을'):
				line_tail = line_tail[:-1]
		elif line_cmd == '넣어라':
			if line_tail.endswith('에'):
				line_tail = line_tail[:-1]
	...
	def preloop(self):
		...
		self.keywords['chn'] = ['打印', '輸入', '材料', '定義', '個兒', '片尾', '万一', '期間', '下次', '走', '那时', '到', '阶段']

		"We will show the current mode and some initial conditions if any."
		self.mode_map = {'eng_kor_mixed': 'eng', 'kor_only': 'kor', \
